...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        # Group to get counts of success(1) and failure(0)
        site_df = filtered_df.groupby(['Launch Site', 'class']).size().reset_index(name='class count')
        fig = px.pie(site_df, values='class count', 
        names='class', 
        title=f"Total Success Launches for site {entered_site}")
        return fig

# TASK 4: Callback function for dropdown and slider as inputs, scatter chart as output
